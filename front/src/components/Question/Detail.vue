    ...
    >
      ...
      <div v-if="q.pt_id === 2" class="answer-container">
        <div v-for="a in q.answers" :key="a.a_id">
          <input type="radio" class="input" :id="a.a_id" v-model="userAnswer" :value="a.a_value" :disabled="!buttonFlag">
          <label :for="a.a_id" class="label no_highlights">{{a.a_value}}</label>        
        </div>
      </div>
      <!-- O / X -->
      ...
      <!-- 정답 체크 -->
      <button class="btn check-btn no_highlights" v-if="buttonFlag" @click="checkProblem(i, q.pt_id, q.answers, q.p_id)" :disabled="!userAnswer">정답 확인</button>

      <div class="current" v-if="!buttonFlag && q.pt_id === 4"> 정답 : {{ answerList[i].join(', ') }}</div>
      <div class="commentary" v-if="!buttonFlag">
        해설 : {{ q.p_commentary }}
      </div>
      <!-- 다음 문제로 -->
      <button class="btn no_highlights" v-if="!buttonFlag && (i !== (questionList.length-1))" @click="nextProblem(i, questionList.length - 1)">다음 문제</button>
      <!-- 오답노트로-->
      <button class="btn wrongnote-btn no_highlights" v-if="!buttonFlag && (i === (questionList.length-1))" @click="goToWrongNote">오답 노트로 이동</button>
    </div>
  </div>
</template>
